...
/**
@title Trigger
@dev This contract represents a Trigger ERC721 token contract, allowing users to create and manage categories.
*/
contract Trigger is ERC721, Ownable  {    
     ...
     */
    event TriggerMinted(uint256 collectionId, uint256 indexed tokenId, string tokenURI);
    ...
    /**
    @dev Initializes the Trigger contract by setting the token name and symbol.
    */
    ...
    modifier tokenExists(uint256 tokenId) {
        require(_exists(tokenId), "Trigger: does not exist");
        _;
    }
    ...
    */
    modifier onlyMinter() {
        require(
            ...
            "Trigger: You are not the Minting Contract :)"
        );
        _;
    }
    ...
    */
    function _burn(
        uint256 tokenId
    ) internal virtual override(ERC721) tokenExists(tokenId) {
        super._burn(tokenId);
    }
    ...
    */
    ...
    /**
    @dev Creates a new trigger token.
