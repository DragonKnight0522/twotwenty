...
/**
 * @title CraftingCard
 * @dev This contract represents a CraftingCard ERC721 token contract, allowing users to create and manage crafting cards.
 */
contract CraftingCard is ERC721, Ownable {    
    ...
    address private predictorContract; // Address of the crafting contract
     ...
     */
    event CraftingMinted(uint256 collectionId, uint256 indexed tokenId, string tokenURI);
    ...
    event PredictorContractChanged(
        address indexed previousCrafter,
        address indexed newCrafter
    );    
    ...
    */
    ...
    modifier onlyCrafting() {
        require(
            msg.sender == craftingContract || msg.sender == predictorContract,
    ...
    modifier onlyMinter() {
        require(
            ...
            "Crafting Card: You are not the Minting Contract :)"
        );
        _;
    }
    ...
    /**
     * @dev Creates a new crafting card token.
    ...
    */
    ...
    function changePredictor(address _newCrafter) public onlyOwner {
        require(_newCrafter != address(0), "Crafting: No zero address");
        address oldCrafter = predictorContract;
        predictorContract = _newCrafter;
        emit PredictorContractChanged(oldCrafter, _newCrafter);
    }
...
*/
    ...
    function changeToCrafted2(
        uint256 tokenId
    ) public tokenExists(tokenId) {
        require(
            msg.sender == predictorContract,
            "You are not the Predictor Contract :)"
        );
        crafted[tokenId] = true;
        _burn(tokenId);
        emit PartCrafted(tokenId);
