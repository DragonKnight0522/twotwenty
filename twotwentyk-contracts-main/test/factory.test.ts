...
const cardPackCreated = new ethers.utils.Interface(["event cardPackCreated(address cardpack)"]);
const collectionCreated = new ethers.utils.Interface(["event CollectionCreated(uint256 tokenId, string tokenURI, address pack, address category, address year, address daymonth, address crafting, address trigger)"]);
...
describe("Category", () => {
  ...
  beforeEach(async () => {
    ...
    const addressNewOwner = await newOnwer.getAddress();

    console.log('owner', addressOwner)
    console.log('new owner', addressNewOwner)
  ...
  describe("createCategory", () => {
    it("should create a new category collection", async () => {
        ...
        await collection.connect(owner).changeMinter(await owner.getAddress());
        ...
        const pack = new ethers.Contract(event.args.pack, cardPackABI, owner)  
        const category = new ethers.Contract(event.args.category, cardABI, owner)
        const year = new ethers.Contract(event.args.year, cardABI, owner)
        const daymonth = new ethers.Contract(event.args.daymonth, cardABI, owner)
        const crafting = new ethers.Contract(event.args.crafting, cardABI, owner)
        const trigger = new ethers.Contract(event.args.trigger, cardABI, owner)
        ...
        await pack.connect(owner).changeAdmin(await owner.getAddress())
        await pack.connect(owner).createStandardCard('https://example.com/standard_pack/0');
        await pack.connect(owner).changeMinter(packOpener.address);
        ...
        await packOpener.openPack(
          ...
          ['https://exmaple.com/collection/0/crafting/0'],
        ...
        console.log('crafting card owner', await crafting.ownerOf(0))
